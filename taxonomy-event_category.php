...
$page_url = get_term_link(get_queried_object(), 'event_category');
...
<div class="archive-feed container">
	...
	<div class="archive-feed__container">
		<?php if ( $view=='month' ){ ?>
			<?php cs__getEventsCalendar($current_term->term_id); ?>

		<?php } else { ?>
			<div class="grid grid--ai-start">
				<div class="archive-feed__sidebar col col--4 col--sm-12 col--xs-12">
					<?php cs__getEventsCalendar($current_term->term_id, 'mini'); ?>
					...
					<?php if ( !empty($taxonomy_event_category) ){ ?>
						<ul class="feed-tags">
							...
							<?php foreach ( $taxonomy_event_category as $id=>$name ){ ?>
								<li class="feed-tags__item<?= ( $id==$current_term->term_id ) ? ' is-active' : ''; ?>">
